...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div
      className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-[9999] flex items-center gap-3 px-5 py-3 rounded-2xl text-white shadow-2xl text-sm font-semibold ${colors[type] || colors.info}`}
      style={{ animation: 'toastSlideUp .25s ease' }}
    >
...
const Translate = () => {
  ...
  useEffect(() => {
    ...
    if (passedCaptions?.length) {
      const normalized = passedCaptions.map(normalize).sort((a, b) => a.start_time - b.start_time);
      setCaptions(normalized);
      setActiveCaptionId(normalized[0].id);
    ...
    const load = async () => {
      ...
      try {
        ...
        const mapped = raw.map(normalize).sort((a, b) => a.start_time - b.start_time);
        setCaptions(mapped);
            ...
            <div className="flex flex-col gap-4 p-6 bg-[#F4F6F8] rounded-b-3xl h-[600px] overflow-y-auto custom-scrollbar">
              {isLoadingCaptions ? (
                ...
                <div
                    ...
                    className={`relative flex items-center gap-4 bg-white rounded-xl px-6 py-4 shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer border min-w-max ${
                  ...
                  {['Professional', 'Casual', 'Creative'].map((tone) => (
                    <button key={tone} onClick={() => setActiveTone(tone)}
                      className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-all duration-300 ${
                        activeTone === tone ? 'bg-brand-button text-white shadow-md' : 'bg-[#F4F6F8] text-brand-navy hover:bg-slate-200'
                      }`}>
                      {tone}
                    </button>
                  ))}
